...
public class ReorderLogs{
  ...
  { 
    ...
    public int compare(String s1, String s2) {
      ...
      return s1.substring(spc1 + 1, s1.length())
      .compareToIgnoreCase(s2.substring(spc2 + 1, s2.length())) == 0 ? 
    s1.substring(0,spc1 - 1).compareToIgnoreCase(s2.substring(0, spc2-1)) : 
    s1.substring(spc1 + 1, s1.length())
      .compareToIgnoreCase(s2.substring(spc2 + 1, s2.length()));
    }
  } 
